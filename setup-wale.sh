...
echo "$WALE_WABS_ACCOUNT_NAME" > /etc/wal-e.d/env/WABS_ACCOUNT_NAME
echo "$WALE_WABS_ACCESS_KEY" > /etc/wal-e.d/env/WABS_ACCESS_KEY
echo "$WALE_WABS_SAS_TOKEN" > /etc/wal-e.d/env/WABS_SAS_TOKEN
...
then
  ...
  su - postgres -c "crontab -l | { cat; echo \"0 4 * * * /usr/bin/envdir /etc/wal-e.d/env /usr/local/bin/wal-e delete --confirm retain 30\"; } | crontab -"
fi
